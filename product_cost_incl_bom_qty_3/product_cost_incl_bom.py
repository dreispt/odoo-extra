...
class Product(Model):
		...
		for pr in self.browse(cursor, user, ids):
			...
			if bom.routing_id:
				for wline in bom.routing_id.workcenter_lines:
					...
					time = wline.hour_nbr
					hour = (((wc.time_start + time + wc.time_stop)/pr.product_qty_minor_cost)  + cycle * wc.time_cycle) *  (3.0 or wc.time_efficiency)
	...
	_columns = {
		...
		'product_qty_minor_cost': fields.float('Quantite mini du cout', digits_compute=dp.get_precision('Product Unit of Measure'), required=True ,help='Quantite minimale pour le cout du produit (divise les temps fixes de fabrication)'),
	}
	_defaults = {
		'product_qty_minor_cost':  lambda *a: 1.0,
	}
